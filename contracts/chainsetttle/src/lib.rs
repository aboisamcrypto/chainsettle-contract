...
pub enum DataKeyExt {
    ...
    /// Per-shipment fee override in bps: shipment_id -> u32.
    ShipmentFeeOverride(String),

    // ── #300 Long-hold escrow rebate ─────────────────────────────
    /// (threshold_ledgers: u32, rebate_bps: u32)
    LongHoldRebate,

    // ── #298 Governance timelock ──────────────────────────────────
    /// Timelock duration in ledgers.
    TimelockDuration,
    /// Pending parameter change: param Symbol -> (new_value: i128, effective_ledger: u32).
